...
// Computes a destination lat/lng given a start point, bearing (degrees) and distance (meters).
// Used purely for DISPLAY geometry (drawing a visual siding/fork line at each wesel on the
// Leaflet map) - has no effect on kp-based physics/collision logic.
...
interface TrainKinematicsSnapshot {
  ...
  zone?: 1 | 2 | 3 | 4; // rute lingkar Sektor 4 (Pasar Senen) memakai geometri peta terpisah
}
 ...
 */
function computeCarPosition(kp: number, direction: LineDirection, idx: number, zone?: 1 | 2 | 3 | 4): { coords: [number, number]; bearing: number } {
  if (idx === 0) {
    return { coords: getLatLngFromKp(kp, direction, zone), bearing: getBearingAtKp(kp, direction, zone) };
  }
  ...
  let lastCoords = getLatLngFromKp(kp, direction, zone);

  const step = 0.0012; // step of about 0.8 meters
  const maxIterations = 80;
  let iterations = 0;

  let coords = getLatLngFromKp(kp, direction, zone); // pre-fallback
  let bearing = getBearingAtKp(kp, direction, zone); // pre-fallback

  const minKp = zone === 4 ? 3.8 : 0;
  const maxKp = zone === 4 ? 9.6 : 54.82;
  ...
  while (accumulatedDist < targetDist && iterations < maxIterations) {
    ...
    if (currentKp < minKp || currentKp > maxKp) {
      break;
    }

    const nextCoords = getLatLngFromKp(currentKp, direction, zone);
...
export default function RealWorldMap({
  ...
  useEffect(() => {
    ...
    if (focusKp !== undefined) {
      ...
      const stationCoords = getLatLngFromKp(focusKp, undefined, focusZone);
    ...
    const targetCoords = getLatLngFromKp(targetZoneKp, undefined, selectedZone as 1 | 2 | 3 | 4);
    mapRef.current.setView(targetCoords, 14, { animate: true }); // zoom out slightly so they can view the entire sector
  }, [selectedZone, focusKp, focusZone, isMapReady]);
  ...
  const drawStaticTracks = () => {
    ...
    // --- Zone 4 Loop Track (Pasar Senen - Gang Sentiong - Kramat - Pondok Jati - Matraman) ---
    // Jalur terpisah secara geografis dari koridor utama (lihat TRACK_GPS_POINTS_ZONE4).
    // Sebelumnya tidak pernah digambar sendiri, sehingga seluruh entitas Zona 4 tampak
    // menumpuk pada jalur utama Jakarta Kota - Manggarai.
    const zone4Coords: [number, number][] = [];
    for (let kp = 3.8; kp <= 9.6; kp += 0.02) {
      zone4Coords.push(getLatLngFromKp(kp, undefined, 4));
    }
    if (!perfMode) {
      L.polyline(zone4Coords, {
        color: theme === 'cyber' ? '#1e293b' : '#334155',
        weight: 5,
        opacity: 0.85
      }).addTo(trackGroup);
    ...
    L.polyline(zone4Coords, {
      color: theme === 'cyber' ? '#a78bfa' : '#7c3aed',
      weight: perfMode ? 1.8 : 1.2,
      opacity: 1
    }).addTo(trackGroup);
  ...
  const drawStations = () => {
    ...
    STATIONS.forEach(station => {
      const coords = getLatLngFromKp(station.kp, undefined, station.zone);
  ...
  useEffect(() => {
    ...
    if (pjlGroup) {
      ...
      pjls.forEach(p => {
        const coords = getLatLngFromKp(p.kp, undefined, p.zone);
    ...
    if (signalGroup) {
      ...
      signals.forEach(sig => {
        ...
        const coords = getLatLngFromKp(sig.kp, sig.direction, sig.zone);
    ...
    if (switchGroup && switches) {
      ...
      switches.forEach(sw => {
        const coords = getLatLngFromKp(sw.kp, sw.direction, sw.zone);
  ...
  useEffect(() => {
    ...
    trains.forEach(tr => {
      kinematicsRef.current[tr.id] = {
        ...
        ts: now,
        zone: tr.routeBranch === 'CikarangLoop' ? 4 : undefined
      };
    });
    ...
    trains.forEach(tr => {
      ...
      coachIndices.forEach((idx) => {
        ...
        const trZone: 1 | 2 | 3 | 4 | undefined = tr.routeBranch === 'CikarangLoop' ? 4 : undefined;
        const { coords, bearing } = computeCarPosition(tr.kp, tr.direction, idx, trZone);
  ...
  useEffect(() => {
    ...
    const smoothTick = () => {
      ...
      (Object.entries(kinematicsRef.current) as [string, TrainKinematicsSnapshot][]).forEach(([trainId, kin]) => {
        ...
        const minKp = kin.zone === 4 ? 3.8 : 0;
        const maxKp = kin.zone === 4 ? 9.6 : 54.8;
        predictedKp = Math.max(minKp, Math.min(maxKp, predictedKp));

        for (let idx = 0; idx < MAX_CARS_RENDERED; idx++) {
          const marker = markerCache[`${trainId}-${idx}`];
          if (!marker) continue;
          const { coords } = computeCarPosition(predictedKp, kin.direction, idx, kin.zone);
